        ...
        {isProduct.map((item) => (
          <SwiperSlide className="" key={item.id}>
            <div className="w-[190px] flex flex-col items-center justify-between gap-2 h-[300px] rounded-xl bg-white py-3 px-2">
              ...
              <div>
                <h2 className="text-[14px] h-10 overflow-hidden">
                  {item.name}
                </h2>
                <del className="text-[14px] text-gray-500">
                  {item.price.toLocaleString()} so'm
                </del>
                <p className="text-[16px] text-[#006bff]">
                  {item.discount_price.toLocaleString()} so'm
                </p>
                <button className="w-[100%] h-[35px] bg-[#006bff] rounded-lg text-white">
                  Yangi
                </button>
              </div>
            </div>
